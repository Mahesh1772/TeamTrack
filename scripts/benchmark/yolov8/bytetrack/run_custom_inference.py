...
def evaluate_predictions(predictions, ground_truth, frame_count):
    ...
    iou_threshold = 0.3  # Reduced from 0.5 to be more lenient
    total_iou = 0
    position_errors = []
...
def run_inference_on_videos():
    ...
    clips = [f'Clip{i}' for i in range(4, 13)]
        ...
        try:
            ...
            for frame_idx, r in enumerate(results, 1):
                ...
                if len(r.boxes) > 0:
                    ...
                    ball_dets = r.boxes[r.boxes.cls == 0]  # assuming ball is class 0
                    if len(ball_dets) > 0:
                        conf_scores = ball_dets.conf
                        best_idx = conf_scores.argmax()
                        box = ball_dets.xywh[best_idx].cpu().numpy()  # convert to numpy array
                        # Debug print
